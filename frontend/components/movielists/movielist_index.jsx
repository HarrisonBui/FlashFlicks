...
class Movielists extends React.Component {
  ...
  selectList(idx) {
    ...
    const shownMovies = shownMovielist.movies.map(id => (this.props.movies[id]));

    this.setState({selectedList: idx, selectedListTitle: shownMovielist.title, shownMovies});
  }

  deleteList() {
    this.props.deleteMovielist(this.props.movielists[this.state.selectedList]);
  }
  ...
  handleSubmit(e) {
    ...
    this.props.createMovielist(newList).then(() => this.setState({modalOpen: false, newListTitle: ''}));
  }

  render() {
    const deleteButton = (
      <button className='delete-button' title='delete list' onClick={this.deleteList}>x</button>
    );

    const movielists = this.props.movielists.map((movielist, idx) => (
        <div key={idx} onClick={() => this.selectList(idx)} className='movielist'>
      ...
      <div className='movielist-view'>
        ...
        <Modal isOpen={this.state.modalOpen} onRequestClose={() => this.setState({modalOpen: false})} className='modal' style={modalStyle} contentLabel='Modal'>
            ...
            <form className='movielist-form'>
              <input placeholder='Title' onChange={this.handleInput} value={this.state.newListTitle} className='new-movielist-form'></input>
